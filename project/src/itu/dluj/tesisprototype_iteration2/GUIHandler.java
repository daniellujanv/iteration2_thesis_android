...
public class GUIHandler {
	...
	private boolean imagesBtnClicked;
	...
	public GUIHandler(int width, int height, Context context){
		...
		patientOneCoords[0] = new Point(screenWidth*0.05, screenHeight*0.25);
		patientOneCoords[1] = new Point(screenWidth*0.48, screenHeight*0.85);
		patientOneCoords[2] = new Point(screenWidth*0.04, screenHeight*0.24);
		patientOneCoords[3] = new Point(screenWidth*0.48, screenHeight*0.85);
		patientOneCoords[4] = new Point(screenWidth*0.09, screenHeight*0.35);
		//PatientTwo Coords
		patientTwoCoords = new Point[5];
		patientTwoCoords[0] = new Point(screenWidth*0.52, screenHeight*0.25);
		patientTwoCoords[1] = new Point(screenWidth*0.95, screenHeight*0.85);
		patientTwoCoords[2] = new Point(screenWidth*0.51, screenHeight*0.24);
		patientTwoCoords[3] = new Point(screenWidth*0.95, screenHeight*0.85);
		patientTwoCoords[4] = new Point(screenWidth*0.56, screenHeight*0.35);
		//PatientInfo Coords
	...
	public Mat drawPatientsToSelect(Mat mRgb){
		...
		rec = writeToImage(rec, patientOneCoords[4], "Patient One", 0.8);
		//Doc Two
		...
		rec = writeToImage(rec, patientTwoCoords[4], "Patient Two", 0.8);
	...
	public Mat drawPatientImages(Mat mRgb){
		...
		rec = writeToImage(rec, patientImgsCoords[4], "Image "+ (currentImg + 1) + " out of "+ numberImgs);
...
//		Log.i("GUIHandler","rec::"+rec.submat(patientImgsRoi).size().toString()+" mBitmap::"+mBitmap.size().toString());
		Core.addWeighted(rec.submat(patientImgsRoi), 0.0, mPatientImages[currentImg], 1.0, 0, rec.submat(patientImgsRoi));
		Mat output = new Mat();
		Core.addWeighted(mRgb, 0.5, rec, 0.5, 0, output);
		return output;
	}
...
//		Log.i("GUIHandler","rec::"+rec.submat(patientImgsRoi).size().toString()+" mBitmap::"+mBitmap.size().toString());
		Core.addWeighted(rec.submat(fullScreenImgRoi), 0.0, mFullScreenImages[currentImg], 1.0, 0, rec.submat(fullScreenImgRoi));
		Mat output = new Mat();
		Core.addWeighted(mRgb, 0.5, rec, 0.5, 0, output);
		return output;
	}
